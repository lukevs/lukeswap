...
contract UniswapPool {
    ...
    function removeLiquidity(uint256 lpTokenAmount) external {
        (uint256 firstTokenAmount, uint256 secondTokenAmount) = _getPoolOwnership(lpTokenAmount);
        lpToken.burn(msg.sender, lpTokenAmount);
        _transferTokensToSender(msg.sender, firstTokenAmount, secondTokenAmount);
    }
    ...
    function _getPoolOwnership(uint256 lpTokenAmount)
        private
        view
        returns (uint256 firstTokenAmount, uint256 secondTokenAmount)
    {
        ...
        return (
            (totalFirstTokenInPool.mul(lpTokenAmount)).div(lpTokenSupply),
            (totalSecondTokenInPool.mul(lpTokenAmount)).div(lpTokenSupply)
        );
    }

    function _transferTokensToSender(
        address sender,
        uint256 firstTokenAmount,
        uint256 secondTokenAmount
    ) private {
        firstToken.safeTransfer(sender, firstTokenAmount);
        secondToken.safeTransfer(sender, secondTokenAmount);
    }
    ...
    function _calculateLPTokenAmountToMint(uint256 firstTokenAmount, uint256 secondTokenAmount)
        private
        view
        returns (uint256)
    {
        ...
        if (lpTokenSupply == 0) {
            ...
            uint256 secondTokenAmountAddedAsFirstToken =
                secondTokenAmount.mul(totalFirstTokenInPool).div(totalSecondTokenInPool);
            uint256 totalValueAddedAsFirstToken = firstTokenAmount.add(secondTokenAmountAddedAsFirstToken);

            uint256 totalFutureSupply =
                lpTokenSupply.mul(totalValueInPoolAsFirstToken).div(
                    (totalValueInPoolAsFirstToken.sub(totalValueAddedAsFirstToken))
                );
